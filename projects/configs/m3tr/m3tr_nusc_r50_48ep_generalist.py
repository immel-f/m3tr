...
map_classes = ['divider_dashed', 'divider_solid', 'boundary', 'centerline', 'ped_crossing']
...
# what type of prior scenario model is trained is specified by the labels, e.g. here a generalist 
ann_root = '/workspace/m3tr/gen_labels_nuscenes/generalist_all_data_geo_split/'
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=ann_root + 'nuscenes_map_infos_temporal_train.pkl',
        #z_cfg=z_cfg,
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='/workspace/m3tr/gen_labels_nuscenes/centerline_only_geo_split/nuscenes_map_infos_temporal_val.pkl',
...
# NOTE: mmcv has a bug in this version that will produce an error during eval for the IterBasedRunner
# If you want to use the IterBasedRunner like here you will need to comment out line 153+154 in 
# /opt/conda/lib/python3.8/site-packages/mmcv/runner/hooks/logger/text.py

epoch_iters = 870 * 7
total_epochs = 48
eval_rate = 1.0 / 1.75

evaluation = dict(interval=round(epoch_iters * eval_rate), pipeline=test_pipeline, metric='chamfer')
# total_epochs = 50
# evaluation = dict(interval=1, pipeline=test_pipeline)



runner = dict(type='IterBasedRunner', max_iters=total_epochs*epoch_iters)
...
checkpoint_config = dict(interval=round(epoch_iters * eval_rate))
