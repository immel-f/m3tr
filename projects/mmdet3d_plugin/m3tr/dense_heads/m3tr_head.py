...
from mmdet.core import (multi_apply, build_assigner, reduce_mean)
...
class M3TRHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.use_masked_map_queries = use_masked_map_queries
        self.mask_assign_gt_one2many = mask_assign_gt_one2many
        self.use_gt_pos_queries = use_gt_pos_queries
        self.use_gt_ref_points = use_gt_ref_points
        self.use_learned_gt_queries = use_learned_gt_queries
        ...
        super(M3TRHead, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.loss_pts = build_loss(loss_pts)
        self.loss_dir = build_loss(loss_dir)
        # manually build assigner for test mode
        if 'test_cfg' in kwargs:
            assigner = kwargs['test_cfg']['assigner']
        ...
        if not self.as_two_stage:
            ...
            if self.query_embed_type == 'all_pts':
                self.query_embedding = nn.Embedding(self.num_query,
                                                    self.embed_dims * 2)
                if self.use_masked_map_queries:
                    self.gt_query_embedding = nn.Embedding(self.num_query,
                                                        self.embed_dims)
                ...
                if self.use_masked_map_queries:
                    self.gt_query_embedding = None
                    self.gt_instance_embedding = nn.Embedding(self.num_vec_one2one, self.embed_dims * 2)
                    self.gt_pts_embedding = nn.Embedding(self.num_pts_per_vec, self.embed_dims * 2)
    ...
    def init_weights(self):
        ...
        if self.query_embed_type == 'instance_pts' and self.use_masked_map_queries:
            nn.init.constant_(self.gt_instance_embedding.weight, 0)
            nn.init.constant_(self.gt_pts_embedding.weight, 0)
        elif self.query_embed_type == 'all_pts' and self.use_masked_map_queries:
            nn.init.constant_(self.gt_query_embedding.weight, 0)
    
    # @auto_fp16(apply_to=('mlvl_feats'))
    @force_fp32(apply_to=('mlvl_feats', 'prev_bev'))
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False, gt_bboxes_3d=None, gt_labels_3d=None, gt_masked=None):
        """Forward function.
        Args:
        ...
        if len(gt_masked) > 0 and self.use_masked_map_queries:

            gt_vecs_list = copy.deepcopy(gt_bboxes_3d)
            device = gt_labels_3d[0].device
            gt_pts_list = [gt_bboxes.fixed_num_sampled_points.to(device)[...,0:2] if not self.z_cfg['gt_z_flag'] else gt_bboxes.fixed_num_sampled_points.to(device) for gt_bboxes in gt_vecs_list]
            gt_pts_list_normalized = [normalize_2d_pts(pts[..., 0:2], self.pc_range) if not self.z_cfg['gt_z_flag'] else normalize_3d_pts(pts, self.pc_range) for pts in gt_pts_list]
        
            if self.query_embed_type == 'instance_pts':
                # print("using instance_pts embed type")

                gt_pts_embeds = self.gt_pts_embedding.weight.unsqueeze(0)
                gt_instance_embeds = self.gt_instance_embedding.weight[0:num_vec].unsqueeze(1)
                gt_object_query_embeds = (gt_pts_embeds + gt_instance_embeds).flatten(0, 1).to(dtype)

                # print((pts_embeds + instance_embeds).shape)
                # print(gt_pts_embeds.shape)
                # print(gt_instance_embeds.shape)
                # print(gt_object_query_embeds.shape)
                
            elif self.query_embed_type == 'all_pts':
                # print("using all_pts embed type")
                gt_object_query_embeds = self.gt_query_embedding.weight.to(dtype)
        else:
            gt_pts_list = None
            gt_pts_list_normalized = None
            gt_object_query_embeds = None
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            ...
            outputs = self.transformer(
                ...
                gt_pts_list_normalized=gt_pts_list_normalized,
                gt_labels=gt_labels_3d,
                gt_masked=gt_masked,
                gt_object_query_embed=gt_object_query_embeds,
                mask_assign_gt_one2many=self.mask_assign_gt_one2many,
                use_gt_pos_queries=self.use_gt_pos_queries,
                use_gt_ref_points=self.use_gt_ref_points,
                use_learned_gt_queries=self.use_learned_gt_queries,
                num_vec_one2many=self.num_vec_one2many,
    ...
    def _get_target_single(self,
        ...
        assign_result, order_index = self.assigner.assign(bbox_pred, cls_score, pts_pred,
                                             gt_bboxes, gt_labels, gt_shifts_pts, gt_masked,
                                             gt_bboxes_ignore)
        
        # self.assigner.assign_new(bbox_pred, cls_score, pts_pred,
        # gt_bboxes, gt_labels, gt_shifts_pts, gt_masked,
        # gt_bboxes_ignore)
    ...
    def get_targets(self,
        ...
        Args:
            ...
            bbox_preds_list (list[Tensor]): Sigmoid outputs from a single
                ...
                (cx, cy, w, h) and shapek_one2manynum_query, 4].
            gt_bboxes_list (list[Tensor]k_one2many Ground truth bboxes for each image
                with shape (num_gts, 4) k_one2many [tl_x, tl_y, br_x, br_y] format.
            gt_labels_list (list[Tensor]k_one2many Ground truth class indices for each
                image with shape (num_gtk_one2many ).
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         ...
         pos_inds_list, neg_inds_list) = multi_apply(
            ...
            gt_labels_list, gt_bboxes_list, gt_shifts_pts_list, gt_masked_list, gt_bboxes_ignore_list)
    ...
    def loss_single(self,
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,pts_preds_list,
                                           ...
                                           gt_masked_list, gt_bboxes_ignore_list)
    ...
    def loss(self,
        ...
        all_gt_masked_list = [gt_masked for _ in range(num_dec_layers)]
        ...
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir = multi_apply(
            ...
            all_gt_bboxes_list, all_gt_labels_list,all_gt_shifts_pts_list, all_gt_masked_list,
            all_gt_bboxes_ignore_list)
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_bbox, enc_losses_iou, enc_losses_pts, enc_losses_dir = \
                self.loss_single(enc_cls_scores, enc_bbox_preds, enc_pts_preds,
                                 gt_bboxes_list, binary_labels_list, gt_pts_list,[gt_masked],gt_bboxes_ignore)
