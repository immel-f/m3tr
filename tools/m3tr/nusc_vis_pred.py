    ...
    for i, data in enumerate(data_loader):
        ...
        for vis_format in args.gt_format:
            if vis_format == 'se_pts':
                ...
                for gt_bbox_3d, gt_label_3d in zip(gt_line_points, gt_labels_3d[0]):
                    ...
                    plt.quiver(x[:-1], y[:-1], x[1:] - x[:-1], y[1:] - y[:-1], scale_units='xy', angles='xy', scale=1, color=colors_plt_dict[label_names[gt_label_3d.item()]])
                ...
                for gt_bbox_3d, gt_label_3d in zip(gt_lines_bbox, gt_labels_3d[0]):
                    ...
                    # import pdb;pdb.set_trace()
                    plt.gca().add_patch(Rectangle(xy,width,height,linewidth=0.4,edgecolor=colors_plt_dict[label_names[gt_label_3d.item()]],facecolor='none'))
                ...
                for gt_bbox_3d, gt_label_3d, masked in zip(gt_lines_fixed_num_pts, gt_labels_3d[0], gt_masked[0]):
                    # import pdb;pdb.set_trace() 
                    ...
                    if masked:
                        plt.plot(x, y, color=colors_plt_dict['masked'],linewidth=1,alpha=0.8,zorder=-1)
                        continue
                    
                    if label_names[gt_label_3d.item()] == 'centerline':
                        plt.quiver(x[:-1], y[:-1], x[1:] - x[:-1], y[1:] - y[:-1], scale_units='xy', angles='xy', scale=1, color=colors_plt_dict[label_names[gt_label_3d.item()]])
                    else:
                        plt.plot(x, y, color=colors_plt_dict[label_names[gt_label_3d.item()]],linewidth=1,alpha=0.8,zorder=-1)
                ...
                for gt_line_instance, gt_label_3d in zip(gt_lines_instance, gt_labels_3d[0]):
                    ...
                    # plt.plot(x, y, color=colors_plt[gt_label_3d])
                    plt.plot(x, y, color=colors_plt_dict[label_names[gt_label_3d.item()]],linewidth=1,alpha=0.8,zorder=-1)
                    # plt.scatter(x, y, color=colors_plt_dict[label_names[gt_label_3d.item()]],s=1,alpha=0.8,zorder=-1)
                plt.imshow(car_img, extent=[-1.2, 1.2, -1.5, 1.5])
        ...
        for pred_score_3d, pred_bbox_3d, pred_label_3d, pred_pts_3d in zip(scores_3d[keep], boxes_3d[keep],labels_3d[keep], pts_3d[keep]):
            ...
            if label_names[pred_label_3d.item()] == 'centerline':
                plt.quiver(pts_x[:-1], pts_y[:-1], pts_x[1:] - pts_x[:-1], pts_y[1:] - pts_y[:-1], scale_units='xy', angles='xy', scale=1, color=colors_plt_dict[label_names[pred_label_3d.item()]])
            else:
                plt.plot(pts_x, pts_y, color=colors_plt_dict[label_names[pred_label_3d.item()]],linewidth=1,alpha=0.8,zorder=-1)
            # plt.scatter(pts_x, pts_y, color=colors_plt_dict[label_names[pred_label_3d.item()]],s=1,alpha=0.8,zorder=-1)
