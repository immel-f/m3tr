...
def _fill_trainval_infos_one_sample(sample_tuple,
    ...
    if 'random_whole_dataset' in masked_elements:
        for masked_elements_random in RANDOM_MASKING_LIST:
            ...
            info_cpy = obtain_vectormap(nusc_maps, map_explorer, info_cpy, point_cloud_range, masked_elements_random)
            info_list.append(info_cpy)
    elif 'random' in masked_elements:
        masked_elements_random = random.choice(RANDOM_MASKING_LIST)
        info = obtain_vectormap(nusc_maps, map_explorer, info, point_cloud_range, masked_elements_random)
        info_list.append(info)
    else:
        info = obtain_vectormap(nusc_maps, map_explorer, info, point_cloud_range, masked_elements)
        info_list.append(info)
...
def _fill_trainval_infos(nusc,
    ...
    try:
        for info_list in tqdm(pool.imap(fn, zip(frame_indices, nusc.sample), chunksize=300)):
...
def obtain_vectormap(nusc_maps, map_explorer, info, point_cloud_range, masked_elements):
    # import ipdb;ipdb.set_trace()
    lidar2ego = np.eye(4)
    ...
    map_anns = vector_map.gen_vectorized_samples(lidar2global_translation, lidar2global_rotation, masked_elements)
    # import ipdb;ipdb.set_trace()
...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, lidar2global_translation, lidar2global_rotation, masked_elements):
        '''
...
def create_nuscenes_infos(root_path,
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_can_bus, nusc_maps, map_explorer, train_scenes, val_scenes, masked_elements, test, max_sweeps=max_sweeps)
...
def nuscenes_data_prep(root_path,
    ...
    create_nuscenes_infos(
        root_path, out_dir, can_bus_root_path, info_prefix, masked_elements, version=version, max_sweeps=max_sweeps)
